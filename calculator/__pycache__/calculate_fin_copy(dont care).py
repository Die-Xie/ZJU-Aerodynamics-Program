...
if __name__ == '__main__':
    ...
    t = Calculator(v=10, alpha=3, sep_loc= na.points, name = na.name, method= 0)
    # t.render_cp() #显示Cp图
    # t = Calculater(v=1, alpha=3, sep_loc= na.points, name = na.name, method= 0)
    t.render_cp() #显示Cp图
    # t.render_streamline() #显示流线图
    
    na = Naca4(naca= '4420',number= 100)
    t = Calculator(v=1, alpha=3, sep_loc= na.points, name = na.name, method= 0)
    t.render_cp()
    plt.show()
